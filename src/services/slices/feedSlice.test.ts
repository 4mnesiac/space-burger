...
describe('check feed reducers', () => {
  it('should turn connection status to open', () => {
    ...
    const result = { ...initialState, wsConnected: true, hasError: false }
  ...
  it('should return error status and payload', () => {
    const reducer = feedReducer(initialState, wsConnectionError())
    const result = { ...initialState, wsConnected: false, hasError: true }
    expect(reducer).toEqual(result)
  })
